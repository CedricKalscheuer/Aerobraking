...
r_p_lowest                  = 85                # Lowest Periapsis tested
r_p_highest                 = 86                # Highest Periapsis tested  
...
def find_optimal_b(initial_b, r_p):
    ...
    b = initial_b
    tolerance = 5  # Allowed difference between r_p and 2nd periapsis of trajectory in km
    max_iterations = 10
    base_max_step = 5.0
    iteration = 0
    error = None
...
def run_full_trajectory(config):
    ...
    if r_p-R <= 80:
        _, _, full_trajectory, _, final_mass = simulate_trajectory(b, r_thrust_descend, r_thrust_ascend, r_p_slow, short_run=False, max_step=max_step)
    elif r_p-R <=90:
        _, _, full_trajectory, _, final_mass = simulate_trajectory(b, r_thrust_descend, r_thrust_ascend, r_p_slow, short_run=False, max_step=max_step*1.5)
    elif r_p-R <=95:
        _, _, full_trajectory, _, final_mass = simulate_trajectory(b, r_thrust_descend, r_thrust_ascend, r_p_slow, short_run=False, max_step=max_step*2.5)
    else:
        _, _, full_trajectory, _, final_mass = simulate_trajectory(b, r_thrust_descend, r_thrust_ascend, r_p_slow, short_run=False, max_step=max_step*6.25)
