...
c_q                         = 0.5               # 0.5 for highest Heat Load (Dachwald fragen)
# === Orbitparameters ============================
tint                        = 120               # Integration time in days
r_p_lowest                  = 75                # Lowest Periapsis tested
r_p_highest                 = 101                # Highest Periapsis tested  
...
Optimize_Thrust_Range       = False              # Optimize R_Thrust_Ascend?
Thrust_Only_Run             = False              # Calculate Thrust Only Run?
Plot_Comparison             = False              # Plot propellant consumed vs time to stable orbit?
Plot_HeatingRate            = True
# === End user input    ============================
#endregion
...
def atmospheric_density(r):
    ...
    H = 195 / np.log(atmo_density / atmo_density_200)   # Scale height based on the model
...
def eom(t, state, thrust, ceff, mdry, r_thrust_descend, r_thrust_ascend, r_p_slow):
    ...
    atmo_density    = atmospheric_density(r)                # Density in [kg/m^3]
...
def find_optimal_b(initial_b, r_p):
    global best_fuel_usage, best_b, best_r_thrust_descend, best_r_thrust_ascend, best_configs, best_trajectory, tolerance
...
def run_full_trajectory(config):
    global stable_orbit_time, aerobrake, slowdown_start_time, stabilization_start_time, max_step_sim
    ...
    thrust_only = config['thrust_only']                          #Accurate=0.2, semi-accurate 0.5 tolerance = 5
    ...
    if r_p-R <= 80:
        _, _, full_trajectory, _, final_mass = simulate_trajectory(b, r_thrust_descend, r_thrust_ascend, r_p_slow, short_run=False, max_step=max_step_sim)
    elif r_p-R <=90:
        _, _, full_trajectory, _, final_mass = simulate_trajectory(b, r_thrust_descend, r_thrust_ascend, r_p_slow, short_run=False, max_step=max_step_sim*1.5)
    elif r_p-R <=95:
        _, _, full_trajectory, _, final_mass = simulate_trajectory(b, r_thrust_descend, r_thrust_ascend, r_p_slow, short_run=False, max_step=max_step_sim*2.5)
    else:
        _, _, full_trajectory, _, final_mass = simulate_trajectory(b, r_thrust_descend, r_thrust_ascend, r_p_slow, short_run=False, max_step=max_step_sim*6.25)
...
if __name__ == "__main__":
    ...
    if Plot_Values:
        ...
        parameter_labels = ['Orbital Energy', 'Altitude', 'Apoapsis', 'Periapsis', 'Mass', 'Velocity', 'Heating Rate']
        ...
        def plot_selected_configuration(config_index, thrust_index):
            global current_config_index, atmospheric_phases
            ...
            if 0 <= config_index < len(full_trajectory_results_sorted):
                ...
                E = v ** 2 / 2 - mu / r  # Orbital Energy
                atmo_density = np.array([atmospheric_density(rad) for rad in r])  # Atmospheric density over the orbit
                Q_dot = c_q * np.sqrt(atmo_density) * v ** 3  # Full-resolution Heating Rate using Q_dot formula

                # Subsampled versions for the other parts of the function that require efficient plotting
                ...
                def update_plot(label):
                    ...
                    if 'Heating Rate' in selected_params:
                        ...
                        ax.yaxis.set_major_formatter(ScalarFormatter())  # Use ScalarFormatter for better tick labels
                        shade_thrust_active(ax, time_sub, mass_sub)
                        shade_atmosphere_regions(ax, time_sub, height_sub, atmo_height)
    ...
    if Plot_Trajectory:
        ...
        def plot_trajectory_for_selected_config(config_index):
            ...
            if 0 <= config_index < len(full_trajectory_results):
                ...
                plot_radius = R_SOI / 1000  # Define the range based on the Sphere of Influence
                ...
                # Manually position the x and y labels at the 0 point on each axis
                x_min, x_max = ax.get_xlim()
                y_min, y_max = ax.get_ylim()
                ax.text(0, y_min * 1.05, "x [$10^3$ km]", ha='center', va='top', fontsize=10) # Position x-label below the 0 mark on the x-axis
                ax.text(x_min * 1.05, 0, "y [$10^3$ km]", ha='right', va='center', rotation='vertical', fontsize=10) # Position y-label left of the 0 mark on the y-axis
            plt.draw()
    ...
    if Plot_HeatingRate:
        ...
        def compute_heating_rate_data(results):
            ...
            unique_configs = sorted({(result['r_thrust_ascend'], result['r_p']) for result in results})  # Sort the configurations
            colors = plt.cm.viridis(np.linspace(0, 1, len(unique_configs)))  # Use a colormap for distinct colors
            ...
            for result in results:
                ...
                v = np.sqrt(rhor ** 2 + (r * rhophi) ** 2)  # Velocity in [km/s]
                atmo_density = np.array([atmospheric_density(rad) for rad in r])  # Atmospheric density over the orbit
                Q_dot = c_q * np.sqrt(atmo_density) * (v) ** 3  # Calculate Heating Rate using Q_dot formula in [W/m^2]

                # Track the max Q_dot, average Q_dot, and the duration of each atmospheric phase
                in_atmosphere = height <= atmo_height
                start = None
                Q_dot_max = 0.0
                Q_dot_sum = 0.0  # Sum of Q_dot values over the phase
                duration_above_threshold = 0.0  # Duration where Q_dot is above 90% of max
                total_duration = 0.0  # Track total duration of the atmospheric phase

                for i, in_atmo in enumerate(in_atmosphere):
                    if in_atmo:
                        if start is None:  # Entering the atmosphere
                            start = time[i]
                        Q_dot_max = max(Q_dot_max, Q_dot[i])  # Update Q_dot_max during this phase
                        Q_dot_sum += Q_dot[i]  # Sum up Q_dot for average calculation
                    elif start is not None:  # Leaving the atmosphere
                        end = time[i]
                        duration = end - start
                        ...
                        config = (result['r_thrust_ascend'], result['r_p'])  # Configuration tuple
                        ...
                        start = None
                        Q_dot_max = 0.0  # Reset for the next phase
                        Q_dot_sum = 0.0
                        duration_above_threshold = 0.0

                if start is not None:  # Handle case where the spacecraft is still in the atmosphere at the end
                    end = time[-1]
                    duration = end - start
                    total_duration = duration
                    Q_dot_avg = Q_dot_sum / (i + 1 - np.argmax(in_atmosphere))

                    for j in range(np.argmax(in_atmosphere), len(time)):
                        if Q_dot[j] >= 0.9 * Q_dot_max:
                            duration_above_threshold += time[j + 1] - time[j]
        ...
        def plot_heating_rate_vs_duration():
            ...
            plt.subplots_adjust(right=0.8)  # Reduce the space between plot and legend

            # Extract the data for plotting
            Q_dot_values = [phase[0] for phase in atmospheric_phases] 
            Q_dot_avg_values = [phase[1] for phase in atmospheric_phases]  
            durations = [phase[2] * 1440 for phase in atmospheric_phases]  # Convert durations from days to minutes
            durations_above = [phase[3] * 1440 for phase in atmospheric_phases]  # Peak durations
            colors = [phase[4] for phase in atmospheric_phases]

            # Plot each atmospheric phase as a scatter point for Q_dot_max and Q_dot_avg
            ...
            plt.ylabel('Max Heating Rate (W/m^2?)')
            plt.title('Max and Average Heating Rate vs Atmospheric Phase Duration')

            plt.axhline(28000, color='black', linestyle='--', linewidth=1, label='Space Shuttle Radiation Limit')  
            plt.axhline(5000, color='red', linestyle='--', linewidth=1, label='TGO Insulation Limit')  
            plt.axhline(2800, color='salmon', linestyle='--', linewidth=1, label='TGO Peak Heat Flux')  
            plt.axhline(1120, color='blue', linestyle='--', linewidth=1, label='TGO Target Peak Heat Flux') 


            # Legend for configurations
            from matplotlib.patches import Patch
            sorted_configs = sorted(color_map.items(), key=lambda x: (x[0][0], x[0][1]))
            legend_elements = [
                Patch(facecolor=color, edgecolor='none', label=f'r_thrust = {config[0] - R:.0f} km, r_p = {config[1] - R:.0f} km')
                for config, color in sorted_configs
            ]
            config_legend = plt.legend(handles=legend_elements, bbox_to_anchor=(1.05, 1), loc='upper left', title='Configurations')

            # Create a second legend for marker symbols
            from matplotlib.lines import Line2D
            marker_legend = plt.legend(
                handles=[
                    Line2D([0], [0], color='red', linestyle='--', linewidth=1, label='TGO Insulation Limit'),
                    Line2D([0], [0], color='orange', linestyle='--', linewidth=1, label='TGO Peak Heat Flux'),
                    Line2D([0], [0], color='blue', linestyle='--', linewidth=1, label='TGO Target Peak Heat Flux')
                ],
                loc='lower left', bbox_to_anchor=(1.05, 0.2), title='Limits'
            )
            # Create a third legend for marker symbols
            marker_legend = plt.legend(
                handles=[
                    Line2D([0], [0], marker='x', color='black', markerfacecolor='black', markersize=8, label='Max Heating Rate'),
                    Line2D([0], [0], marker='o', color='w', markerfacecolor='black', markersize=8, label='Average Heating Rate')
                ],
                loc='lower left', bbox_to_anchor=(1.05, 0), title='Markers'
            )

            plt.gca().add_artist(config_legend)  # Add the configuration legend back
